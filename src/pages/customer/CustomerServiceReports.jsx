...
import { FileText, Printer, Download, CheckCircle2, ShieldCheck, UserCheck, Search, HardDrive } from 'lucide-react';
import { COMPANY_INFO } from '../../data/websiteData';

export default function CustomerServiceReports() {
  const { activeCustomer, serviceReports } = useAuth();
  const customerReports = serviceReports.filter(r => r.customerNo === activeCustomer?.customerNo);
  const [selectedReport, setSelectedReport] = useState(customerReports[0] || null);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs flex flex-col sm:flex-row justify-between sm:items-center gap-4 no-print">
        <div>
          <h2 className="text-lg font-bold text-slate-900">Service Reports & Inspection Logs</h2>
          <p className="text-xs text-slate-500">
            Verified CCTV inspection sheets conducted on your premises.
          </p>
        </div>
        ...
        {selectedReport && (
          <button
            onClick={handlePrint}
            className="px-4 py-2.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs flex items-center space-x-1.5 shadow-xs transition-colors self-start sm:self-auto"
          >
            <Printer className="w-4 h-4 text-gold-400" />
            <span>Print Official Service Report (PDF)</span>
          </button>
        )}
      </div>

      {/* Reports Selector Tabs if multiple (No Print) */}
      {customerReports.length > 1 && (
            ...
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition-all border ${
                selectedReport?.id === rep.id
                  ? 'bg-gold-500 text-white border-gold-500 shadow-gold-soft'
                  : 'bg-white text-slate-700 border-slate-200 hover:bg-slate-50'
              }`}
            >
              Report #{rep.reportNo} ({rep.date})
            </button>
          ))}
        </div>
      )}

      {/* Official CCTV Service Report Document (Exact reproduction of PDF 1) */}
      {selectedReport ? (
        <div id="printable-service-report" className="bg-white p-8 sm:p-10 rounded-2xl border border-slate-300 shadow-soft-lg space-y-6 text-slate-900 font-sans">
          ...
          <div className="space-y-2">
            ...
            <div className="border border-slate-400 rounded overflow-hidden text-xs">
              <div className="grid grid-cols-2 sm:grid-cols-4 divide-x divide-y sm:divide-y-0 divide-slate-300 p-3 bg-slate-50 gap-2">
                ...
                <div>
                  <span className="text-slate-500 text-[10px] block">Complaint / Service Request:</span>
            ...
            <div className="border border-slate-400 rounded overflow-hidden text-xs">
              <div className="grid grid-cols-2 sm:grid-cols-4 divide-x divide-y sm:divide-y-0 divide-slate-300 p-3 bg-slate-50 gap-2">
                ...
                <div>
                  ...
                  <span className="text-slate-800">{selectedReport.model || 'DS-7616NI-Q2'}</span>
                </div>
                <div>
              ...
              <div className="p-3 border-t border-slate-300 bg-white">
                ...
                <span className="text-slate-800 font-medium">{selectedReport.otherEquipment}</span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="p-3 bg-slate-50 border border-slate-400 rounded text-xs text-slate-800">
          ...
          {/* Parts / Materials Used Table from PDF 1 */}
          <div className="space-y-2">
          ...
          {/* System Inspection Status (3 Badges) */}
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            <div className="p-3 bg-slate-50 border border-slate-400 rounded text-xs text-slate-800 italic">
          ...
          <div className="pt-6 border-t-2 border-slate-300 space-y-4 text-xs">
            ...
            <div className="grid grid-cols-2 gap-8 pt-4">
              <div className="border-t border-slate-500 pt-1.5">
                ...
                <p className="text-[11px] text-slate-500">Date: {selectedReport.date}</p>
              </div>
        ...
        <div className="p-8 bg-white rounded-2xl border border-slate-200 text-center">
          <p className="text-xs text-slate-500">No service reports available yet.</p>
        </div>
      )}
